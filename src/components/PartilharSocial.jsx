...
/**
 * PartilharSocial - Botoes de partilha para WhatsApp, Facebook, e copiar link
 * Suporta Web Share API quando disponivel
 */
export default function PartilharSocial({
  url,
  titulo = 'Sete Ecos - Sistema de Transmutacao Feminina',
  texto = 'Descobre os Sete Ecos, uma jornada de transformacao feminina.',
  compact = false,
  className = ''
}) {
  ...
  const textoEncoded = encodeURIComponent(texto + ' ' + shareUrl);
  ...
  if (compact) {
    return (
      <div className={`flex gap-2 ${className}`}>
        <a
          href={`https://wa.me/?text=${textoEncoded}`}
          target="_blank"
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      <p className="text-sm font-semibold text-gray-500 uppercase tracking-wider">Partilhar</p>
      <div className="flex flex-wrap gap-3">
        <a
          href={`https://wa.me/?text=${textoEncoded}`}
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center gap-2 px-4 py-2 bg-[#25D366] text-white rounded-full text-sm font-medium hover:bg-[#20BD5A] transition-colors"
        >
        ...
        <button
          onClick={copiarLink}
          className="flex items-center gap-2 px-4 py-2 bg-gray-600 text-white rounded-full text-sm font-medium hover:bg-gray-700 transition-colors"
        >
