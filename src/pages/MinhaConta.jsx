...
export default function MinhaConta() {
  ...
  const getStatusBadge = (status) => {
    const badges = {
      active: { bg: 'bg-emerald-500/15', text: 'text-emerald-700', label: 'Activo', dot: 'bg-emerald-500' },
      tester: { bg: 'bg-violet-500/15', text: 'text-violet-700', label: 'Tester', dot: 'bg-violet-500' },
      trial: { bg: 'bg-sky-500/15', text: 'text-sky-700', label: 'Trial', dot: 'bg-sky-500' },
      pending: { bg: 'bg-amber-500/15', text: 'text-amber-700', label: 'Pendente', dot: 'bg-amber-500' },
      expired: { bg: 'bg-red-500/15', text: 'text-red-700', label: 'Expirado', dot: 'bg-red-500' },
      cancelled: { bg: 'bg-gray-500/15', text: 'text-gray-600', label: 'Cancelado', dot: 'bg-gray-400' }
    };
      ...
      <div className="flex items-center gap-3 p-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="font-semibold text-[#4A3728] text-sm">{name}</span>
            {badge && (
          ...
          {sub?.subscription_expires && (
            <p className="text-[#6B5344]/40 text-xs">Expira: {formatDate(sub.subscription_expires)}</p>
          )}
          {!sub && <p className="text-[#6B5344]/40 text-xs">Sem subscricao</p>}
        </div>
        {hasAccess ? (
      ...
      <div className="max-w-lg mx-auto">
        ...
        <header className="px-5 pt-8 pb-2">
          <div className="flex items-center justify-between">
            <button onClick={() => navigate('/')} className="w-10 h-10 bg-white/50 backdrop-blur-sm rounded-2xl flex items-center justify-center hover:bg-white/70 transition-colors shadow-sm border border-white/60">
        ...
        <section className="px-5 py-6">
          <div className="bg-white/50 backdrop-blur-md rounded-3xl border border-white/60 shadow-lg p-5">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <h1 className="text-xl font-bold text-[#4A3728] truncate" style={{ fontFamily: "'Cormorant Garamond', serif" }}>
                  {displayName}
                </h1>
                <p className="text-[#6B5344]/50 text-xs truncate">{user?.email}</p>
                <p className="text-[#6B5344]/30 text-[10px] mt-1">
        ...
        <section className="px-5 mb-6">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="text-sm font-semibold text-[#4A3728] tracking-widest uppercase">Os meus Ecos</h2>
          </div>
          ...
          <div className="bg-white/50 backdrop-blur-md rounded-3xl border border-white/60 shadow-lg overflow-hidden divide-y divide-[#E8D5A3]/15">
            ...
            <div className="flex items-center gap-3 p-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="font-semibold text-[#4A3728] text-sm">LUMINA</span>
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-violet-500/15 text-violet-700">Gratuito</span>
                </div>
                <p className="text-[#6B5344]/40 text-xs">Diagnostico diario</p>
              </div>
        ...
        <section className="px-5 mb-6">
          ...
          <div className="bg-white/50 backdrop-blur-md rounded-3xl border border-white/60 shadow-lg overflow-hidden divide-y divide-[#E8D5A3]/15">
            ...
            <Link to="/recuperar-password" className="flex items-center gap-3 p-3.5 hover:bg-white/40 transition-colors">
              <div className="w-9 h-9 bg-[#6B5344]/10 rounded-xl flex items-center justify-center flex-shrink-0">
                <svg className="w-4 h-4 text-[#6B5344]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <button onClick={handleLogout} className="w-full flex items-center gap-3 p-3.5 hover:bg-red-50/40 transition-colors">
        ...
        <div className="text-center px-5 pb-4">
          <p className="text-[#6B5344]/30 text-xs mb-1">Precisas de ajuda?</p>
          <a href="mailto:suporte@seteecos.com" className="text-[#C9A227]/60 text-xs font-medium hover:text-[#C9A227] transition-colors">
